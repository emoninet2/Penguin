...
void update_time(){
	...
	_nrf24l01p_send_to_address_ack(0x1122334455, (uint8_t*)txData,strlen(txData));
	_delay_ms(500);
...
void thread_3( void *pvParameters ){
	_nrf24l01p_init();
	_nrf24l01p_set_RX_pipe_address(_NRF24L01P_PIPE_P1, 0x5858585831);
	_nrf24l01p_set_RX_pipe_address(_NRF24L01P_PIPE_P2, 0x5858585832);
	_nrf24l01p_set_RX_pipe_address(_NRF24L01P_PIPE_P3, 0x5858585833);
	_nrf24l01p_set_RX_pipe_address(_NRF24L01P_PIPE_P4, 0x5858585834);
	_nrf24l01p_set_RX_pipe_address(_NRF24L01P_PIPE_P5, 0x5858585835);
	
	_nrf24l01p_set_frequency_offset(2);
	...
	while(1){
		...
		//char txData[] = "\t\t\tL";
		sprintf(txbuff,"X %d",total_count);
		_nrf24l01p_send_until_ack(0x4541525448, (uint8_t*)txbuff,strlen(txbuff));
		total_count++;
		//_delay_ms(5);
...
// 			rxData[width] = '\0';
// 				
// 			//_nrf24l01p_write_to_address_ack(0xC3C3C3C3FF, (uint8_t*)rxData,width);	
// 					
// 			ssd1306_clear();
// 
// 			ssd1306_set_page_address(0);
...
// 			//_nrf24l01p_write_to_address_ack(0x1122334455, (uint8_t*)rxData,width);
// 			//mesh_pack_forward_test(rxData);
// 		}
...
int main(void)
{
	...
	xTaskCreate(thread_3,(const char *) "t3", 200, NULL, tskIDLE_PRIORITY, NULL );
